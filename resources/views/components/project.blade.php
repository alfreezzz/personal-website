    ...
    <div class="justify-between flex my-5">
        <div>
            @if(request()->query('token') === env('ADMIN_ACCESS_TOKEN'))
                <x-btn-add href="{{ url('portofolio/create') . '?' . http_build_query(['token' => request()->query('token')]) }}">
                    project
                </x-btn-add>
            @endif
        </div>
    ...
    @if($projects->isEmpty())
        <div class="flex flex-col max-w-screen-mobile-m justify-center items-center mx-auto">
            ...
            <p class="text-center text-gray-400 mb-4 mt-2 font-light sm:text-base text-sm">
                Project has not yet been added.
            </p>
            <x-cta-btn href="{{ url('portofolio/create') . '?' . http_build_query(['token' => request()->query('token')]) }}"
                >Add a project
            </x-cta-btn>
        </div>
    @else
            ...
            @foreach ($projects as $project)
                <div>
                    <div class="flex max-lg:flex-col justify-between items-start">
                        <a href="{{ $project->url ?? url('not-found'). '?' . http_build_query(['token' => request()->query('token')]) }}" 
                        class="lg:w-1/3 w-full" target="_blank">
                        ...
                        <div class="lg:w-2/3 w-full lg:ml-6">
                            <div class="flex items-center justify-between mb-2 max-lg:mt-3">
                                ...
                                class="hover:underline" target="_blank">
                                    <h2 class="sm:text-2xl text-lg font-semibold lg:mr-6 mr-4">
                                        {{ $project->nama_apk }}
                                    </h2>
                                </a>
                                <h3 class="sm:text-lg text-gray-400 text-right">
                                    {{ $project->tgl_selesai }}
                                </h3>
                            </div>
                            <a href="{{ url()->current() . '?' . http_build_query(array_merge(request()->query(), ['jenis_apk' => $project->jenis_apk])) }}#project" 
                            class="px-3 py-1 rounded-xl bg-[#0077C0] lg:text-sm text-xs font-medium hover:bg-[#005a99] transition duration-200">
                                {{ $project->jenis_apk }}
                            </a>
                            <p class="mt-5 text-gray-300" id="project-description-{{ $project->id }}">
                                ...
                                @if (strlen(strip_tags($project->deskripsi)) > 125)
                                    <a href="javascript:void(0)" class="font-semibold text-[#C7EEFF] hover:underline read-more" data-id="{{ $project->id }}">
                                        Read More
                                    </a>
                                @endif
                            </p>                    
                            <div class="lg:mt-7 mt-5 flex justify-between items-center">
                                <div class="grid grid-cols-5 mobile-m:grid-cols-6 mobile-l:grid-cols-7 sm:grid-cols-12">
                                        ...
                                        <div class="rounded-full border border-[#C7EEFF] hover:translate-y-1 sm:w-10 sm:h-10 w-8 h-8 hover:bg-[#C7EEFF] transition duration-200 flex justify-center items-center">
                                            @switch($bahasa)
                                                ...
                                                @case('Javascript')
                                                    ...
                                                    @break
                                                @case('PHP')
                                                    <svg role="img" class="sm:w-5 sm:h-5 h-4 w-4" fill="#777BB4" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                        <title>PHP</title>
                                                    ...
                                                    @break
                                                @case('Figma')
                                                    <svg role="img" class="sm:w-5 sm:h-5 h-4 w-4" fill="#F24E1E" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                        <title>Figma</title>
                                                    ...
                                                    @break
                                                @case('Laravel')
                                                    <svg role="img" class="sm:w-5 sm:h-5 h-4 w-4" fill="#FF2D20" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                        <title>Laravel</title>
                                                    ...
                                                    @break
                                                @case('Tailwind CSS')
                                                    <svg role="img" class="sm:w-5 sm:h-5 h-4 w-4" fill="#06B6D4" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                        <title>Tailwind CSS</title>
                                                        <path d="M12.001,4.8c-3.2,0-5.2,1.6-6,4.8c1.2-1.6,2.6-2.2,4.2-1.8c0.913,0.228,1.565,0.89,2.288,1.624 C13.666,10.618,15.027,12,18.001,12c3.2,0,5.2-1.6,6-4.8c-1.2,1.6-2.6,2.2-4.2,1.8c-0.913-0.228-1.565-0.89-2.288-1.624 C16.337,6.182,14.976,4.8,12.001,4.8z M6.001,12c-3.2,0-5.2,1.6-6,4.8c1.2-1.6,2.6-2.2,4.2-1.8c0.913,0.228,1.565,0.89,2.288,1.624 c1.177,1.194,2.538,2.576,5.512,2.576c3.2,0,5.2-1.6,6-4.8c-1.2,1.6-2.6,2.2-4.2,1.8c-0.913-0.228-1.565-0.89-2.288-1.624 C10.337,13.382,8.976,12,6.001,12z"/>
                                                    </svg>
                                                    @break
                                                @case('Alpine.js')
                                                    <svg role="img" class="sm:w-5 sm:h-5 h-4 w-4" fill="#8BC0D0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                        <title>Alpine.js</title>
                                                        <path d="m24 12-5.72 5.746-5.724-5.741 5.724-5.75L24 12zM5.72 6.254 0 12l5.72 5.746h11.44L5.72 6.254z"/>
                                                    </svg>
                                                    @break
                                                @case('React')
                                                    <svg role="img" class="sm:w-5 sm:h-5 h-4 w-4" fill="#61DAFB" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                        <title>React</title>
                                                    ...
                                                    @break
                                                @case('Next.js')
                                                    <svg role="img" class="sm:w-5 sm:h-5 h-4 w-4" fill="#ffffff" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                        <title>Next.js</title>
                                                        <path d="M18.665 21.978C16.758 23.255 14.465 24 12 24 5.377 24 0 18.623 0 12S5.377 0 12 0s12 5.377 12 12c0 3.583-1.574 6.801-4.067 9.001L9.219 7.2H7.2v9.596h1.615V9.251l9.85 12.727Zm-3.332-8.533 1.6 2.061V7.2h-1.6v6.245Z"/>
                                                    </svg>
                                                    @break
                                                @case('Python')
                                                    <svg role="img" class="sm:w-5 sm:h-5 h-4 w-4" fill="#3776AB" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                        <title>Python</title>
                                ...
                                <a href="{{ $project->url ?? url('not-found'). '?' . http_build_query(['token' => request()->query('token')]) }}" 
                                class="sm:text-lg text-sm mobile-m:text-base underline hover:no-underline decoration-2 decoration-solid decoration-[#0077C0] underline-offset-8 font-medium text-[#0077C0] hover:border-2 border-[#0077C0] sm:px-4 px-2 py-1 rounded-md hover:text-[#C7EEFF] transition" target="_blank">
                                Live preview
                                </a>
                            </div>
                        </div>
