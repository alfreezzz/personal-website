    ...
    <div class="flex flex-col lg:gap-16 gap-20 mt-5 mx-auto">
        ...
        <div class="">
            <div class="flex max-lg:flex-col justify-between items-start">
                <a href="{{ $project->url ?? url('not-found'). '?' . http_build_query(['token' => request()->query('token')]) }}" class="lg:w-1/3 w-full" target="_blank">
                ...
                <div class="lg:w-2/3 w-full lg:ml-6">
                    <div class="flex items-center justify-between mb-2 max-lg:mt-3">
                        <a href="{{ $project->url ?? url('not-found'). '?' . http_build_query(['token' => request()->query('token')]) }}" class="hover:underline" target="_blank">
                    ...
                    <div class="lg:mt-7 mt-5 flex justify-between items-center">
                        ...
                        <a href="{{ $project->url ?? url('not-found'). '?' . http_build_query(['token' => request()->query('token')]) }}" class="sm:text-lg text-sm mobile-m:text-base underline hover:no-underline decoration-2 decoration-solid decoration-[#0077C0] underline-offset-8 font-medium text-[#0077C0] hover:border-2 border-[#0077C0] sm:px-4 px-2 py-1 rounded-md hover:text-[#C7EEFF] transition" target="_blank">Live preview</a>
                    </div>
                </div>
            </div>
            @if(request()->query('token') === env('ADMIN_ACCESS_TOKEN'))
