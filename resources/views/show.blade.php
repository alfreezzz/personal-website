    ...
    <div id="blog" class="xl:px-32 px-3 sm:px-5 mt-32 flex max-lg:flex-col lg:justify-between">
        <div class="grow-2 shrink lg:basis-[65%] basis-full lg:pr-5">
            <div class="leading-relaxed">
                <h1 class="sm:text-4xl text-2xl font-bold tracking-wide mb-1">
                    {{ $currentArticle->judul_artikel }}
                </h1>
                <div class="mb-4 mt-2">
                    <p class="sm:text-sm text-xs font-light tracking-wider">
                        {{ \Carbon\Carbon::parse($currentArticle->created_at)->timezone('Asia/Jakarta')->format('d M Y') }} • 
                ...
                <img src="{{ asset('storage/' . $currentArticle->img) }}" 
                     alt="Article image"
                     class="w-full max-h-[350px] object-cover lg:mb-8 mb-3 lg:mt-5 mt-4 rounded-lg">
                <p class="tracking-wider text-justify text-base leading-relaxed whitespace-pre-line">
                    {{ $currentArticle->isi_artikel }}
                </p>
            </div>
                ...
                @foreach ($relatedArticles as $related)
                    <div class="flex mb-5 items-center">
                        ...
                        <div class="text-left ml-5">
                            <a href="{{ url('blog/' . $related->slug) . '?' . http_build_query(['token' => request()->query('token')]) }}" 
                               class="m-0 hover:underline font-medium tracking-wide leading-[1.3] mb-2 sm:text-base text-xs">
                                    {{ $related->judul_artikel }}
                            </a>
                            <p class="mr-1 my-0 sm:text-sm text-xs font-light tracking-wider">
