<x-layout-2 :title="$title">
    <div id="blog" class="xl:px-32 px-5 mt-28">
        ...
        <form action="{{ url('blog') }}" method="POST" enctype="multipart/form-data" class="grow-2 shrink basis-full lg:pr-5">
            @csrf
            <div class="leading-relaxed">
                <div class="flex justify-between">
                    ...
                    <div class="relative" x-data="{ isOpen: false, selected: 'Article' }">
                        <button
                            type="button"
                            @click="isOpen = !isOpen"
                            class="flex lg:tracking-widest tracking-wide sm:tracking-wider items-center justify-between w-full lg:px-4 px-3 py-1 lg:text-base text-sm text-[#C7EEFF] bg-black rounded-md shadow ring-2 ring-inset ring-[#c7eeff2f] hover:bg-gray-800"
                ...
                <input type="text" name="judul_artikel" placeholder="Title" class="border border-[#c7eeff2f] bg-black w-full rounded-md px-2 py-1 focus:outline-none focus:ring-2 focus:ring-[#0077C0]">
                @error('judul_artikel')
                    <span class="text-sm">{{ $message }}</span>
                @enderror
